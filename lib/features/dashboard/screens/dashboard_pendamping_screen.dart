...
class DashboardPendampingScreen extends StatelessWidget {
  ...
  // HOME TAB
  // ═══════════════════════════════════════════════════════════════════════════
  Widget _buildHome(BuildContext context, HajiCareController state, DashboardController dashboardCtrl) {
  ...
  // HERO SECTION
  // ═══════════════════════════════════════════════════════════════════════════
  Widget _hero(BuildContext context, HajiCareController state, DashboardController dashboardCtrl,
      PrayerTimesController prayerCtrl, bool isDark) {
    final topInset = MediaQuery.paddingOf(context).top;

    final displayName = state.pendampingName.value.isNotEmpty
        ? state.pendampingName.value
        : (state.self.name.trim().isNotEmpty ? state.self.name.trim() : 'Pendamping');
    ...
    final prayerName = prayerCtrl.nextPrayerName.value.isNotEmpty ? prayerCtrl.nextPrayerName.value : 'Ashar';
    final cleanTime = (prayerCtrl.nextPrayerTime.value.isNotEmpty ? prayerCtrl.nextPrayerTime.value : '15:20').split(' ').first;
        ...
        children: [
                ...
                child: Column(
                  ...
                  children: [
                    Text("Assalamu'alaikum,",
                        style: TextStyle(color: Colors.white.withValues(alpha: 0.72), fontSize: 12.5, fontWeight: FontWeight.w500)),
                  ...
                  children: [
                    Row(
                      ...
                      children: [
                        Text('$jamaahCount',
                            style: const TextStyle(
                                color: Colors.white, fontSize: 42, fontWeight: FontWeight.w800, letterSpacing: 0.5, height: 1.0)),
          ...
          Row(
            ...
            children: [
              ...
              _metricCol(value: '$prayerName $cleanTime', label: 'Jadwal Salat'),
                ...
                Expanded(
                  child: Material(
                    ...
                    child: InkWell(
                      ...
                      onTap: () { HapticFeedback.selectionClick(); dashboardCtrl.changeTab(1); },
                      child: Center(
                        child: Row(
                          mainAxisSize: MainAxisSize.min,
                          children: const [
                            Icon(Icons.north_west_rounded, color: Colors.white, size: 16),
                            SizedBox(width: 7),
                            Text('Lacak Jamaah',
                ...
                Expanded(
                  child: Material(
                    ...
                    child: InkWell(
                      ...
                      onTap: () { HapticFeedback.mediumImpact(); NotificationComposerDialog.show(context); },
                      child: Center(
                        child: Row(
                          mainAxisSize: MainAxisSize.min,
                          children: const [
                            Text('Broadcast Notif',
                                style: TextStyle(color: Colors.white, fontSize: 13, fontWeight: FontWeight.w700)),
                            SizedBox(width: 7),
  ...
  // CURVED BODY
  // ═══════════════════════════════════════════════════════════════════════════
  Widget _body(BuildContext context, HajiCareController state, DashboardController dashboardCtrl, bool isDark) {
    ...
    return Container(
      ...
      decoration: BoxDecoration(
        ...
        boxShadow: [BoxShadow(color: Colors.black.withValues(alpha: isDark ? 0.4 : 0.08), blurRadius: 18, offset: const Offset(0, -6))],
      ),
      padding: const EdgeInsets.fromLTRB(20, 24, 20, 110),
      child: Obx(() {
        final selectedJamaah = (state.jamaahList.isNotEmpty &&
          ...
          children: [
            ...
            Container(
              ...
              child: Row(
                ...
                crossAxisAlignment: CrossAxisAlignment.start,
                children: [
                  _iconItem(context: context, label: 'Kelola Room', icon: Icons.meeting_room_rounded, isDark: isDark,
                      onTap: () => state.activeRoomId.value != null
                          ? Get.toNamed(AppRoutes.roomDetail, arguments: state.activeRoom.value)
                          : Get.toNamed(AppRoutes.joinRoom)),
                  _iconItem(context: context, label: 'Undang Jamaah', icon: Icons.person_add_alt_1_rounded, isDark: isDark,
                      onTap: () => Get.toNamed(AppRoutes.joinRoom)),
                  _iconItem(context: context, label: 'Jadwal Salat', icon: Icons.access_time_rounded, isDark: isDark,
                      onTap: () => dashboardCtrl.changeTab(2)),
                  _iconItem(context: context, label: 'Darurat SOS', icon: Icons.emergency_rounded, isDark: isDark,
                      iconColor: AppColors.sosEmergency, onTap: () => Get.toNamed(AppRoutes.modalSos)),
                ],
              ),
            ),
            ...
            if (state.anySosActive || state.anyJamaahSeparated) ...[
              _sectionHeader(title: 'Status Darurat', actionText: 'Lihat peta',
                  ...
                  onDismissSos: (jamaahId) async {
                    AppAlert.confirm(context,
                        ...
                        onConfirm: () async {
                          ...
                          if (context.mounted) {
                            AppAlert.success(context, title: 'SOS Diakhiri', message: 'Sinyal darurat berhasil dinonaktifkan.');
                          }
                        });
                  },
                ),
              if (state.anyJamaahSeparated) _separatedAlert(context, state, isDark),
              const SizedBox(height: 26),
            ],

            // Monitored Pilgrims Pills
            if (state.activeRoomId.value != null && state.jamaahList.isNotEmpty) ...[
              _sectionHeader(title: 'Pantauan Jamaah', actionText: 'Lihat peta',
                  onAction: () => dashboardCtrl.changeTab(1), headingColor: headingColor, isDark: isDark),
              const SizedBox(height: 12),
            ...
            // Kamar & Maktab
            _sectionHeader(title: 'Kamar & Maktab', actionText: 'Kelola',
                onAction: () => state.activeRoomId.value != null
                    ? Get.toNamed(AppRoutes.roomDetail, arguments: state.activeRoom.value)
                    : Get.toNamed(AppRoutes.joinRoom),
                headingColor: headingColor, isDark: isDark),
            const SizedBox(height: 12),
            ...
            // Tips
            _sectionHeader(title: 'Tips & Panduan Tugas', actionText: 'Selengkapnya',
                onAction: () {}, headingColor: headingColor, isDark: isDark),
            const SizedBox(height: 12),
  ...
  // MONITORED PILGRIMS PILLS
  // ═══════════════════════════════════════════════════════════════════════════
  Widget _pilgrimsPills(BuildContext context, HajiCareController state,
      DashboardController dashboardCtrl, bool isDark) {
    ...
    return Container(
      ...
      decoration: BoxDecoration(
        ...
        border: Border.all(color: isDark ? AppColors.darkCardBorder : AppColors.lightCardBorder, width: 1.2),
      ...
      child: state.jamaahList.isEmpty
          ? Center(child: Padding(
              ...
              child: Text('Belum ada jamaah terdaftar',
                  style: TextStyle(color: isDark ? AppColors.darkTextBody : AppColors.textBody, fontSize: 13)),
            ))
          : Wrap(
              spacing: 8,
              runSpacing: 8,
                    ...
                    child: Row(
                      ...
                      children: [
                        ...
                        if (hasSos) ...[
                          ...
                          const Icon(Icons.emergency_rounded, color: AppColors.sosEmergency, size: 13),
                        ],
                      ],
                    ),
                  ),
  ...
  Widget _notifButton(BuildContext context, HajiCareController state) {
    return Stack(
      ...
      children: [
        ...
        Obx(() {
          final notifCtrl = Get.isRegistered<NotificationController>() ? Get.find<NotificationController>() : null;
          final unread = notifCtrl != null ? notifCtrl.unreadCount.value + notifCtrl.pendingInvitations.length : 0;
          ...
          return Positioned(
            ...
            child: Container(
              width: 10, height: 10,
              decoration: BoxDecoration(
                color: hasSos ? AppColors.sosEmergency : const Color(0xFF00E5FF),
                shape: BoxShape.circle,
                border: Border.all(color: const Color(0xFF2E1C12), width: 1.5),
                boxShadow: [BoxShadow(
                    color: (hasSos ? AppColors.sosEmergency : const Color(0xFF00E5FF)).withValues(alpha: 0.6),
  ...
  Widget _broadcastChip(BuildContext context) {
    return Expanded(
      child: InkWell(
        onTap: () { HapticFeedback.lightImpact(); NotificationComposerDialog.show(context); },
          ...
          children: [
            Container(
              ...
              decoration: BoxDecoration(
                ...
                border: Border.all(color: AppColors.goldLight.withValues(alpha: 0.45), width: 1.0),
              ),
              child: Row(
                mainAxisSize: MainAxisSize.min,
                children: const [
                  Icon(Icons.campaign_rounded, color: AppColors.goldLight, size: 13),
                  SizedBox(width: 4),
                  Text('Broadcast',
                      style: TextStyle(color: AppColors.goldLight, fontSize: 11, fontWeight: FontWeight.w700, letterSpacing: 0.1)),
                ],
              ),
            ),
            const SizedBox(height: 3),
            Text('Kirim Notif',
                style: TextStyle(color: Colors.white.withValues(alpha: 0.65), fontSize: 11, fontWeight: FontWeight.w500),
    ...
    return LayoutBuilder(builder: (context, constraints) {
      ...
      return Row(
        ...
        children: List.generate(dashCount, (_) =>
            SizedBox(width: dashWidth, height: 1.2,
  ...
  Widget _iconItem({
    ...
    final effectiveColor = iconColor ?? (isDark ? AppColors.goldLight : AppColors.espressoDark);
          ...
          children: [
            Container(
              ...
              decoration: BoxDecoration(
                ...
                borderRadius: BorderRadius.circular(18),
                border: Border.all(
                  color: isDark ? AppColors.darkOutlineVariant.withValues(alpha: 0.5) : AppColors.canvasCreamSubtle,
                  width: 1.0,
                ),
                boxShadow: [BoxShadow(
                    color: isDark ? Colors.black.withValues(alpha: 0.25) : AppColors.espressoDark.withValues(alpha: 0.05),
            ...
            SizedBox(
              width: 76,
              child: Text(label,
                  style: TextStyle(color: textColor, fontSize: 11, fontWeight: FontWeight.w600, height: 1.22),
  ...
  Widget _sectionHeader({
    ...
    return Row(
      mainAxisAlignment: MainAxisAlignment.spaceBetween,
      children: [
        Text(title, style: AppTypography.titleMedium.copyWith(fontWeight: FontWeight.w800, color: headingColor, fontSize: 16)),
        InkWell(
          onTap: onAction,
          borderRadius: BorderRadius.circular(8),
          child: Padding(
  ...
  Widget _separatedAlert(BuildContext context, HajiCareController state, bool isDark) {
    return Container(
      margin: const EdgeInsets.only(top: 10),
      padding: const EdgeInsets.all(14),
      decoration: BoxDecoration(
        color: isDark ? AppColors.darkSurfaceContainerHighest : AppColors.surfaceContainerLow,
        borderRadius: BorderRadius.circular(16),
        border: Border.all(color: AppColors.statusWarning.withValues(alpha: 0.6), width: 1.5),
      ),
      child: Row(
        children: [
          Container(
            padding: const EdgeInsets.all(8),
            decoration: BoxDecoration(color: AppColors.statusWarning.withValues(alpha: 0.15), shape: BoxShape.circle),
            child: const Icon(Icons.person_off_rounded, color: AppColors.statusWarning, size: 20),
          ),
  ...
  Widget _tipsCard(bool isDark, Color headingColor) {
    ...
    final tips = [
      (Icons.groups_rounded, 'Absensi Rutin', 'Cek kehadiran jamaah minimal 3× sehari: subuh, zuhur, dan isya.'),
      (Icons.local_hospital_rounded, 'Kondisi Kesehatan', 'Pantau jamaah lanjut usia dan yang memiliki riwayat penyakit kronis.'),
      (Icons.wifi_tethering_rounded, 'Koneksi GPS', 'Pastikan jamaah mengaktifkan GPS agar posisi terpantau secara real-time.'),
    ];
            ...
            children: [
              if (i > 0)
                Padding(
                  ...
                  child: Divider(height: 1, color: isDark ? AppColors.darkCardBorder : AppColors.lightCardBorder),
                ),
              Row(
                crossAxisAlignment: CrossAxisAlignment.start,
                children: [
                  Container(
                    ...
                    child: Icon(icon, color: isDark ? AppColors.goldLight : AppColors.espressoDark, size: 20),
                  ),
                  const SizedBox(width: 12),
                  Expanded(
                    child: Column(
                      crossAxisAlignment: CrossAxisAlignment.start,
                      children: [
                        Text(title, style: TextStyle(color: headingColor, fontSize: 13, fontWeight: FontWeight.w700)),
                        const SizedBox(height: 3),
                        Text(desc,
                            style: TextStyle(
