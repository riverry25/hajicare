...
void main() {
  ...
  group('AppStartupController Tests', () {
    ...
    test(
      'setOnboardingDone marks onboarding as completed persistently',
      () async {
        final controller = Get.put(AppStartupController());
    ...
    test(
      'isRememberMe defaults to true and setRememberMe persists value',
      () async {
        final controller = Get.put(AppStartupController());
    ...
    test(
      'determineInitialRoute returns onboarding when onboarding_done is false',
      () async {
        final controller = Get.put(AppStartupController());
    ...
    test(
      'determineInitialRoute returns login when onboarding_done is true and no user',
      () async {
        SharedPreferences.setMockInitialValues({
