...
void main() {
  group('GeocodingService Search Locations Tests', () {
    test('Query < 2 characters returns empty list without making HTTP request', () async {
      int requestCount = 0;
      ...
      final resultsSingle = await service.searchLocations('j', client: client);
    ...
    test('Photon primary: parses GeoJSON features, coordinates, and structured address', () async {
      final client = MockClient((request) async {
      ...
      final service = GeocodingService();
      final results = await service.searchLocations("McDonald's", client: client);
    ...
    test('Photon primary: applies proximity bias when latitude and longitude provided', () async {
      Uri? capturedUri;
      final client = MockClient((request) async {
    ...
    test('Photon primary: runs global search without lat/lon when coordinates are null', () async {
      Uri? capturedUri;
      final client = MockClient((request) async {
        capturedUri = request.url;
        return http.Response('{"type":"FeatureCollection","features":[]}', 200);
