...
void main() {
  group('Smart Multi-Pass Money Detection — Acceptance Tests', () {
    ...
    test('IoU calculation verifies exact overlap and non-overlap', () {
      ...
      expect(SmartMultiPassDetector.calculateIoU(boxA, boxB), closeTo(1.0, 0.001));

      const boxDisjoint = Rect.fromLTWH(0.5, 0.5, 0.2, 0.2); // disjoint -> IoU = 0.0
      expect(SmartMultiPassDetector.calculateIoU(boxA, boxDisjoint), equals(0.0));
    ...
    test('Test 4: Foto tanpa uang => 0 detections, Total = 0 Riyal', () {
      ...
      final merged = SmartMultiPassDetector.mergeCrossPassDetections([pass1, pass2]);
      final total = calculateTotal(merged);
    ...
    test('Test 5: Same physical object detected by multiple passes merged into 1 with best confidence', () {
      ...
      const boxA2 = Rect.fromLTWH(0.21, 0.30, 0.29, 0.25); // IoU > 0.85 with boxA1
      const boxA3 = Rect.fromLTWH(0.20, 0.31, 0.30, 0.24); // IoU > 0.85 with boxA1
      ...
      final merged = SmartMultiPassDetector.mergeCrossPassDetections([pass1, pass2, pass3]);

      // MUST be exactly 1 object, not 3 duplicate rows
      expect(merged.length, equals(1));
      expect(merged.first.amount, equals(5.0));
      expect(merged.first.confidence, equals(0.93)); // Preserved highest confidence
    });

    test('Test 6: Two physically separate 5 Riyal objects => 2 detections, 10 Riyal (Do NOT merge)', () {
    ...
    test('Test 7: Many money: 5 + 10 + 20 + 5 + 100 + 5 + 10 + 50 => 8 physical objects, TOTAL = 205 Riyal', () {
      final pass1 = [
        createDetection(
          className: 'five riyal',
          confidence: 0.89,
    ...
    test('Multi-pass candidate confirmation: weak candidate (<0.65) confirmed across 2 passes is boosted and kept', () {
    ...
    test('Single unconfirmed weak candidate (<0.65) in only one pass is dropped', () {
      const box = Rect.fromLTWH(0.25, 0.25, 0.30, 0.30);
